...
def battle_quality(outcome: str, party_hp_fraction: float | None, turns: int, *, target_turns: int = 6) -> dict:
    ...
    A win always scores above every draw, loss, or truncation.  Within wins,
    ...
    still trains on the preregistered terminal +1/-1/0 reward.
    """
    if outcome not in {"win", "loss", "draw", "truncated"}:
