...
Use validation for a small preregistered hyperparameter grid (for example KL 0/.01/.05, same search budget for controls). Then lock hyperparameters and run untouched test once at the fixed budget. A null or negative result is valid. No synthetic fixture scores can be used as evidence for this hypothesis.
