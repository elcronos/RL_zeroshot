...
- **119 Python tests** covering policy masking, PPO/GAE, frozen evaluation,
...
- A real mGBA replay produced the same transcript with and without artificial
  inference delay.
- Laya and PrismNLI inference run locally. Jev completed through OpenRouter;
  timeouts, rate limits, and server errors use bounded retries, while malformed
  responses, authentication failures, and other errors fail explicitly.

PrismNLI, Laya, Jev, and uniform each won 18/18 zero-shot test battles. All
three 1,024-step residual policies also completed all 18 battles. Their turns,
final party HP, battle scores, artifact hashes, and wall times are recorded in
[results.md](results.md). The hosted Jev arm completed end to end in 28 minutes
25 seconds.
...
states and multiple preregistered learner seeds at a larger policy budget.
