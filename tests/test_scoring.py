...
def test_outcome_order_is_preserved():
    assert battle_quality("win", 0.0, 100)["battle_score"] > battle_quality("draw", 1.0, 0)["battle_score"]
    assert battle_quality("draw", 1.0, 0)["battle_score"] > battle_quality("loss", 1.0, 0)["battle_score"]
    assert battle_quality("loss", 1.0, 0)["battle_score"] > battle_quality("truncated", 1.0, 0)["battle_score"]


@pytest.mark.parametrize("outcome,hp,turns", [("bad", 0.5, 1), ("win", 1.1, 1), ("win", 0.5, -1)])
